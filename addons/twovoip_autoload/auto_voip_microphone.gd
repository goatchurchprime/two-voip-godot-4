...
signal audio_json_packet_ready(audio_stream_packet_header: Dictionary)
signal audio_packet_ready(opus_packet: PackedByteArray)
...
func _init(
		p_json_packets_as_binary: bool = true,
...
func request_audio_json_packet_mid_header() -> Variant:
	if not is_currently_talking:
		return null

	var audio_stream_packet_mid_header: Dictionary[StringName, Variant] = {
		AutoVoip.PARAM_OPUS_FRAME_SIZE: opus_chunk_size,
		AutoVoip.PARAM_OPUS_SAMPLE_RATE: opus_sample_rate,
		AutoVoip.PARAM_OPUS_CHANNELS: opus_channels,
		AutoVoip.PARAM_LEN_CHUNK_PREFIX: len(chunk_prefix),
		AutoVoip.PARAM_OPUS_STREAM_COUNT: opus_stream_count,
		AutoVoip.PARAM_OPUS_FRAME_COUNT: opus_frame_count - 1,
		AutoVoip.PARAM_TALKING_TIME_START: talking_time_start,
		}
	if json_packets_as_binary:
		return AutoVoip.to_ascii_buffer(audio_stream_packet_mid_header)
	else:
		return audio_stream_packet_mid_header
...
func _process_talk_stream_ends(is_talking: bool) -> void:
	if is_talking and not is_currently_talking:
		...
		var audio_stream_packet_header: Dictionary[StringName, Variant] = {
			AutoVoip.PARAM_OPUS_FRAME_SIZE: opus_chunk_size,
			AutoVoip.PARAM_OPUS_SAMPLE_RATE: opus_sample_rate,
			AutoVoip.PARAM_OPUS_CHANNELS: opus_channels,
			AutoVoip.PARAM_LEN_CHUNK_PREFIX: len(chunk_prefix),
			AutoVoip.PARAM_OPUS_STREAM_COUNT: opus_stream_count,
			AutoVoip.PARAM_OPUS_FRAME_COUNT: 0,
			AutoVoip.PARAM_TALKING_TIME_START: talking_time_start,
			}
		opus_encoder.reset_opus_encoder()

		if json_packets_as_binary:
			audio_packet_ready.emit(AutoVoip.to_ascii_buffer(audio_stream_packet_header))
		else:
			audio_json_packet_ready.emit(audio_stream_packet_header)
		...
		var audio_packet_stream_footer: Dictionary[StringName, Variant] = {
			AutoVoip.PARAM_OPUS_STREAM_COUNT: opus_stream_count,
			AutoVoip.PARAM_OPUS_FRAME_COUNT: opus_frame_count,
			AutoVoip.PARAM_TALKING_TIME_DURATION: talking_time_duration,
			AutoVoip.PARAM_TALKING_TIME_END: talking_time_end,
			}

		if json_packets_as_binary:
			audio_packet_ready.emit(AutoVoip.to_ascii_buffer(audio_packet_stream_footer))
		else:
			audio_json_packet_ready.emit(audio_packet_stream_footer)
		opus_stream_count += 1
...
func _process_opus_chunk():
	...
	audio_packet_ready.emit(opus_packet)
	opus_frame_count += 1
