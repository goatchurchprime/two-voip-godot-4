...
func _initialize() -> void:
	...
	rnnoise_voice.set_gain(0.5)
	assert(rnnoise_voice.process_chunk(make_constant(960, 0.25)) == 960)
	assert(abs(rnnoise_voice.get_peak() - 0.125) < 0.000001)
	assert(abs(rnnoise_voice.get_rms() - 0.125) < 0.000001)
	assert(rnnoise_voice.get_speech_probability() >= 0.0 and rnnoise_voice.get_speech_probability() <= 1.0)
	var rnnoise_agc_voice := TwovoipOpusEncoder.new()
	assert(rnnoise_agc_voice.initialize(48000, 48000, 1, TwovoipOpusEncoder.DENOISER_RNNOISE, TwovoipOpusEncoder.AGC_APPLIED, 960) == OK)
	assert(rnnoise_agc_voice.process_chunk(make_constant(960, 0.25)) == 960)
	assert(rnnoise_agc_voice.get_agc_gain() > 0.0)
	...
	var packet_encoder := TwovoipOpusEncoder.new()
	assert(packet_encoder.initialize(48000, 48000, 1, TwovoipOpusEncoder.DENOISER_DISABLED, TwovoipOpusEncoder.AGC_DISABLED, 960) == OK)
	assert(packet_encoder.create_opus_encoder(20000, 5, true))
	assert(packet_encoder.process_chunk(make_stereo(960)) == 960)
	var prefix := PackedByteArray([42, 24])
	var packet: PackedByteArray = packet_encoder.encode_chunk(prefix)

	var decoder := TwovoipOpusDecoder.new()
	assert(decoder.initialize(44100, 1) == ERR_INVALID_PARAMETER)
	assert(not decoder.is_initialized())
	assert(decoder.initialize(48000, 1) == OK)
	assert(decoder.initialize(48000, 1) == ERR_ALREADY_IN_USE)
	var decoded: PackedFloat32Array = decoder.decode_packet(packet, prefix.size())
	assert(decoded.size() == 960)
	assert(decoder.get_last_frame_size() == 960)
	assert(decoder.get_last_error() == 0)
	assert(decoder.get_last_error_message() == "success")
	assert(decoder.decode_packet(packet, prefix.size(), true).size() == 960)
	assert(decoder.decode_missing(960).size() == 960)
	assert(decoder.decode_missing(5761).is_empty())
	assert(decoder.get_last_error() < 0)
	decoder.reset()
	assert(decoder.get_last_frame_size() == 0)
	assert(decoder.decode_packet(packet, packet.size()).is_empty())
	assert(decoder.get_last_error() < 0)
	assert(decoder.decode_packet(packet, prefix.size()).size() == 960)

	var stereo_decoder := TwovoipOpusDecoder.new()
	assert(stereo_decoder.initialize(48000, 2) == OK)
	assert(stereo_decoder.decode_packet(encoder.encode_chunk()).size() == 1920)

	print("audio processing smoke passed: native voice processing, encode, decode, gain, rates, frames, stereo and mono")
	quit()
