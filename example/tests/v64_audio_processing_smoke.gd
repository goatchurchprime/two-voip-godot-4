...
func _initialize() -> void:
	...
	var deferred_rnnoise := TwovoipOpusEncoder.new()
	assert(deferred_rnnoise.initialize(48000, 48000, 1, TwovoipOpusEncoder.DENOISER_RNNOISE_DEFERRED, TwovoipOpusEncoder.AGC_DISABLED, 960) == OK)
	assert(deferred_rnnoise.process_chunk(make_constant(960, 0.25)) == 960)
	assert(abs(deferred_rnnoise.get_current_chunk()[100].x - 0.25) < 0.000001)
	assert(deferred_rnnoise.get_speech_probability() == 0.0)
	assert(deferred_rnnoise.denoise_chunk(0) == OK)
	assert(abs(deferred_rnnoise.get_current_chunk()[100].x - 0.25) > 0.000001)
	assert(not deferred_rnnoise.encode_chunk(PackedByteArray(), 0).is_empty())
	assert(deferred_rnnoise.denoise_chunk(1000) == ERR_INVALID_PARAMETER)
